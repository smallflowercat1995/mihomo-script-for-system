...
echo "true"
nohup /Applications/Microsoft\ Edge.app/Contents/MacOS/Microsoft\ Edge --user-data-dir="${PWD}/edge-user-data" --proxy-server=http://127.0.0.1:7890 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAxAB" > edge.log 2>&1 &
elif [ -e /Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome ];then 
echo "true"
nohup /Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome --user-data-dir="${PWD}/chrome-user-data" --proxy-server=http://127.0.0.1:7890 "https://www.youtube.com/results?search_query=免费节点&sp=EgQIAxAB" > chrome.log 2>&1 &
else 
